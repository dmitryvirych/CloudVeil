...
- (void)setupCaptureSession
{
	...
	_applicationWillEnterForegroundObserver = [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationWillEnterForegroundNotification object:[UIApplication sharedApplication] queue:nil usingBlock:^(__unused NSNotification *note)
    {
...
- (void)storeThumbnailWithSampleBuffer:(CMSampleBufferRef)sampleBuffer time:(NSTimeInterval)time mirror:(bool)mirror
{
    ...
    if (err != kvImageNoError)
        TGLog(@"Video Message thumbnail generation error %ld", err);
