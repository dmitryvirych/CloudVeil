...
static void *NSTextCheckingResultCloudVeilHiddenLinkKey = &NSTextCheckingResultCloudVeilHiddenLinkKey;

@implementation NSTextCheckingResult (TGMessage)

- (void)setIsCloudVeilHiddenLink:(bool)value {
    objc_setAssociatedObject(self, NSTextCheckingResultCloudVeilHiddenLinkKey, @(value), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (bool)isCloudVeilHiddenLink {
    return [objc_getAssociatedObject(self, NSTextCheckingResultCloudVeilHiddenLinkKey) boolValue];
}
    ...
    if (entities != nil) {
        ...
        for (TGMessageEntity *entity in entities) {
            ...
            if ([entity isKindOfClass:[TGMessageEntityBold class]]) {
                ...
                [result setIsCloudVeilHiddenLink:true];
